...
### User experience
The web API, **ProductService**, maintains an in-memory collection of to-do items per authenticated user. The authenticated user carries the same to-do list across multiple instances of the web app in this sample as well as native clients like the [.NET native (WPF) client](https://github.com/Azure-Samples/active-directory-dotnet-native-aspnetcore).

The ASP.NET Core 2.0 web app, **ProductWebApp** enables a user to:
- Sign in. The first time a user signs in, a consent screen prompts the user for permission to access **ProductService** and obtain user profile information from Azure Active Directory. Since this is a web app, hosted in a browser, it can be that the user gets immediately signed-in benefiting from Single Sign On with other web applications. 
- Select **Product List** in the app's navigation bar. The user may then:
...
![Product WebApp](./Readme/Product-webApp.png)
...
#### Register the service app (ProductService-aspnetcore)

1. Navigate to the Microsoft identity platform for developers [App registrations](https://go.microsoft.com/fwlink/?linkid=2083908) page.
1. Select **New registration**.
1. When the **Register an application page** appears, enter your application's registration information:
   - In the **Name** section, enter a meaningful application name that will be displayed to users of the app, for example `ProductService-aspnetcore`.
...
1. Select the **Expose an API** section, and:
   ...
   - Enter the following parameters
     ...
     - in **Admin consent display name** type `Access ProductService-aspnetcore as a user`
     - in **Admin consent description** type `Accesses the ProductService-aspnetcore Web API as a user`
     - in **User consent display name** type `Access ProductService-aspnetcore as a user`
     - in **User consent description** type `Accesses the ProductService-aspnetcore Web API as a user`
     - Keep **State** as `Enabled`
     - Select **Add scope**

#### Register the client app (ProductWebApp-aspnetcore)

1. Navigate to the Microsoft identity platform for developers [App registrations](https://go.microsoft.com/fwlink/?linkid=2083908) page.
1. Select **New registration**.
1. When the **Register an application page** appears, enter your application's registration information:
   - In the **Name** section, enter a meaningful application name that will be displayed to users of the app, for example `ProductWebApp-aspnetcore`.
   - Change **Supported account types** to **Accounts in any organizational directory**.
...
1. Select the **API permissions** section
   ...
   - In the list of APIs, select the API `ProductService-aspnetcore`.
   - In the **Delegated permissions** section, ensure that the right permissions are checked: **user_impersonation**.
   - Select the **Add permissions** button.

#### Configure authorized client applications for service (ProductService-aspnetcore)

For the middle tier web API (`ProductService-aspnetcore`) to be able to call the downstream web APIs, the user must grant the middle tier permission to do so in the form of consent.
...
1. In the [Azure portal](https://portal.azure.com), navigate to your `ProductService-aspnetcore` app registration, and in the **Expose an API** section, click on **Add a client application**.
   Input the client ID of the client application (`ProductWebApp-aspnetcore`) and check **user_impersonation** for authorized scopes.
...
1. Open the `ProductService\appsettings.json` file
1. Find the app key `Domain` and replace the existing value with your Azure AD tenant name.
1. Find the app key `TenantId` and replace the existing value with your Azure AD tenant ID.
1. Find the app key `ClientId` and replace the existing value with the application ID (clientId) of the `ProductService-aspnetcore` application copied from the Azure portal.

#### Configure the client project

> Note: if you used the setup scripts, the changes below will have been applied for you

1. Open the `ProductWebApp\appsettings.json` file
1. Find the app key `Domain` and replace the existing value with your Azure AD tenant name.
1. Find the app key `TenantId` and replace the existing value with your Azure AD tenant ID.
1. Find the app key `ClientId` and replace the existing value with the application ID (clientId) of the `ProductWebApp-aspnetcore` application copied from the Azure portal.
1. Find the app key `ClientSecret` and replace the existing value with the key you saved during the creation of the `ProductWebApp-aspnetcore` app, in the Azure portal.
1. Find the app key `ProductResourceId` and replace the existing value with `api://{ProductService_ClientId}`.

### Step 4: Run the sample

In the solution properties, set both projects as startup projects. Set **ProductService** to run first. Clean the solution, rebuild it, and then run it.  

On startup, the web API displays an empty web page. This is expected behavior.

Explore the sample by signing in into the web app, clicking on "Product List", signing again if needed, adding items to the To Do list, signing-out, and starting again.  Since the authenticated session is stored in a cookie, the application doesn't require logging in again if the previous session was never signed out.

> [!NOTE]
> The To Do list is stored in memory in this sample. Each time the ProductService API is stopped, any to-do lists are reset.
...
- Update of the AzureAdOptions class to add a property to compute the `Authority` from the `instance` and the `tenantID`, and adding two other configuration options for `ClientSecret`,  the `resourceId` of ProductService (its clientId) and the base address for this service.
- Added a `ProductItem` in models to deserialize the Json sent by the ProductService
...
1. Edit the ProductService's properties (right click on `ProductService.csproj`, and choose **Properties**)
1. In the Debug tab:
    1. Check the **Launch browser** field to `https://localhost:44351/api/Product`
...
The same kind of modifications can be made on the `ProductWebApp.csproj` project.
...
- Step 3: 
    ...
    - Navigate to the appsettings.json files for both the ProductService web API and ProductWebApp web application. Replace the "Instance" property in the Azure AD section with `https://login.microsoftonline.us/`. 
